...
@section('title', 'Market Güncelleme')
...
<!--begin::App Content Header-->
        <div class="app-content-header">
          ...
          <div class="container-fluid">
            <!--begin::Row-->
            <div class="row">
              <div class="col-sm-6"><h3 class="mb-0">Market Güncelleme</h3></div>
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-end">
                  <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Anasayfa</a></li>
                  <li class="breadcrumb-item active" aria-current="page">Market Yönetimi</li>
                </ol>
              </div>
            </div>
            <!--end::Row-->
                ...
                <div class="card">
                    ...
                    <?php 
                        foreach($markets as $market){
                            $market_id[$market->lang] = $market->market_id;
                            $seo_url[$market->lang] = $market->seo_url;
                            $title[$market->lang] = $market->title;
                            $title_2[$market->lang] = $market->title_2;
                            $short_description[$market->lang] = $market->short_description;
                            $description[$market->lang] = $market->description;
                            $image[$market->lang] = $market->image;
                            $icon_image[$market->lang] = $market->icon_image;
                            $alt[$market->lang] = $market->alt;
                            $used_products[$market->lang] = explode(',', $market->used_products) ?? [];
                            $seo_title[$market->lang] = $market->seo_title;
                            $seo_description[$market->lang] = $market->seo_description;
                            $seo_keywords[$market->lang] = $market->seo_keywords;
                            $sort[$market->lang] = $market->sort;
                        }
                    ?>
                    <div class="card-body">
                                ...
                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="tab-{{ $language->id }}" role="tabpanel" aria-labelledby="tab-{{ $language->id }}-tab">
                                    <div class="card-body" style="">
                                        <div class="grids-4">
                                            ...
                                            <input type="hidden" name="market_id" value="{{ $market_id[$language->lang_code] }}">
                                            <div class="mb-3">
                                                <label for="title_{{ $language->lang_code }}" class="form-label">Başlık 1 ({{ $language->lang_code }})</label>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                @if(isset($image[$language->lang_code]))
                                                    <img src="{{ $language->domain .'/'. getFolder(['uploads_folder', 'market_images_folder'], $language->lang_code) . '/' . $image[$language->lang_code] }}" alt="{{ $alt[$language->lang_code] ?? '' }}" class="img-thumbnail mt-2" width="100">
                                                    <input type="hidden" name="old_image_{{ $language->lang_code }}" value="{{ $image[$language->lang_code] }}">
                                                @endif
                                            </div>

                                            <div class="mb-3">
                                                <label for="icon_image_{{ $language->lang_code }}" class="form-label">Görsel ({{ $language->lang_code }})</label>
                                                <input type="file" class="form-control" id="icon_image_{{ $language->lang_code }}" name="icon_image_{{ $language->lang_code }}" accept="image/*"  >
                                                @if(isset($image[$language->lang_code]))
                                                    <img src="{{ $language->domain .'/'. getFolder(['uploads_folder', 'market_images_folder'], $language->lang_code) . '/' . $icon_image[$language->lang_code] }}" alt="{{ $alt[$language->lang_code] ?? '' }}" class="img-thumbnail mt-2" width="100">
                                                    <input type="hidden" name="old_icon_image_{{ $language->lang_code }}" value="{{ $icon_image[$language->lang_code] }}">
                                                @endif
                                            </div>
