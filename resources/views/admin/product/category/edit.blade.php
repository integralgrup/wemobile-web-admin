                            ...
                            <div class="tab-content" id="myTabContent">
                                ...
                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="tab-{{ $language->id }}" role="tabpanel" aria-labelledby="tab-{{ $language->id }}-tab">
                                    <div class="card-body" style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px;">
                                        ...
                                        <div class="mb-3">
                                            <label for="parent_category_id_{{ $language->lang_code }}" class="form-label">Üst Kategori ({{ $language->lang_code }})</label>
                                            <select class="form-select" id="parent_category_id_{{ $language->lang_code }}" name="parent_category_id_{{ $language->lang_code }}">
                                                <option value="">Seçiniz</option>
                                                @foreach($categories as $kk => $parentCategory)
                                                    <option value="{{ $parentCategory->category_id }}"   {{ $parentCategory->category_id == $parent_category_id ? 'selected' : '' }}>{{ $parentCategory->title }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <!-- soru -->
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" id="seo_url_{{ $language->lang_code }}" name="seo_url_{{ $language->lang_code }}" value="{{ $category_items[$key]->seo_url ?? '' }}">
                                        </div>
